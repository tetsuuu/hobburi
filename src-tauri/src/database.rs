...
pub(crate) async fn migrate_database(pool: &SqlitePool) -> DbResult<()> {
  sqlx::migrate!("./migrate").run(pool).await?;
  Ok(())
}
...
pub(crate) async fn move_card(
  ...
  let mut tx = pool.begin().await?;
  remove_card_position(&mut tx, from.column_id, card.id, from.position).await?;
  insert_card_position(&mut tx, to.column_id, card.id, to.position).await?;
  tx.commit().await?;
  Ok(())
}

pub(crate) async fn remove_card(pool: &SqlitePool, card: Card, column_id: i64) -> DbResult<()> {
  let mut tx = pool.begin().await?;
  ...
  remove_card_position(&mut tx, column_id, card.id, position).await?;
