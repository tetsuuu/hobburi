...
async fn handle_remove_card(
    ...
    database::remove_card(&*sqlite_pool, card, column_id)
        .await
        .map_err(|e| e.to_string())?;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    const DATABAE_DIR: &str = "hubburi";
    const DATABAE_FILE: &str = "db.sqlite";

    let home_dir = directories::UserDirs::new()
        .map(|dirs| dirs.home_dir().to_path_buf())
        .unwrap_or_else(|| std::env::current_dir().expect("Cannot access the current directory"));
    let database_dir = home_dir.join(DATABAE_DIR);
    let database_file = database_dir.join(DATABAE_FILE);
    ...
    // std::fs::canonicalizeだとWindowsで動かないので…
    let database_dir_str = dunce::canonicalize(&database_dir)
        .unwrap()
        .to_string_lossy()
        .replace('\\', "/");
    let database_url = format!("sqlite://{}/{}", database_dir_str, DATABAE_FILE);
